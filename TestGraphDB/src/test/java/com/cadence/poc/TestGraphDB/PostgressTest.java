...
public class PostgressTest extends CommonTest {
	public PostgressTest() {
		super("postgress.properties");
	}
	...
	public void testSelectOnTwoVertices()
	{
		...
		GraphTraversal<Vertex, Map<String, Object>> traversal = sqlgGraph.traversal().V()
	            .hasLabel(type1)
	            .has("P1_"+i, "val*").has("P2_"+i,i).has("P3_"+i,1.0*i)
	            .as("A")
	            .out("Edge1")
	            .hasLabel(type2)
	...
	public void showNormalBatchMode() {
	    ...
	    for (int i = 1; i <= 10_000_000; i++) {
	        Vertex person = this.sqlgGraph.addVertex(T.label, "Person", "name", "John" + i,"P1","val_"+i , "P2", i, "P3", 1.0*i, "P4","value"+i , "P5", 111.0*i);
	        Vertex car = this.sqlgGraph.addVertex(T.label, "Car", "name", "Dodge" + i,"R1","val_"+i , "R2", i, "R3", 1.0*i, "R4","value"+i , "R5", 111.0*i);
	...
	@Test
	public void timingForGetByIdforTwoTypesAndFiveColumnsUsingNormalBatchNode()
	{
		for(int N= 10_000_000; N<=10_000_000; N=N*10)
		{
	...
	@Test
	public void timingForSearchByPropertyforTwoTypesAndFiveColumnsUsingNormalBatchNode()
	{
		int N= 10;
			System.out.println("-----Processing "+N+" vertex of two types-----");
			start = System.currentTimeMillis();
